...
import numpy as np
from balanceDegree import edgesBalanceDegree, edgesBalanceDegree_s

network='bitcoin-otc'

args = DotMap(
    train_file_folder='{}/trains'.format(network), # path_to_the_train_file_folder
    test_file_path='{0}/tests/{0}-test-7.csv'.format(network), # path_to_the_test_csv_file
    num_layers=2,
    channels=64,
    lr=1e-2,
    epochs=300,
    seed=2023,
    divisions_number=300,
    initial=0.5
)
...
def run(train_file_name: str):
    ...
    # 计算平衡度
    edges = g_train.T.cpu().numpy()
    m = len(edges)
    node_num = int(max([max(e[:2]) for e in edges])) + 1
    A = np.zeros([node_num, node_num])
    for e in edges:
        i, j, weight = e[0], e[1], e[2]
        A[i][j] = weight
    balanceDegree = edgesBalanceDegree(A)
    balanceDegree = np.array([balanceDegree[e[0]][e[1]] for e in edges])
    ...
    def train(epoch):
        ...
        train_edge_now=torch.tensor(edges[chooseOrder[:num]].T).to(device)
